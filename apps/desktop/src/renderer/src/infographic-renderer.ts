import { assertInfographicSourceSize } from './infographic-policy';
...
const createWorker = (): Worker =>
  new Worker(new URL('./infographic-renderer.worker.ts', import.meta.url), {
    name: 'fuxian-infographic',
    type: 'module',
  });

export const createInfographicRenderer = (
    ...
    while (activeWorkers < maximumConcurrentWorkers && queue.length > 0) {
      ...
      worker.addEventListener('message', (event: MessageEvent<RenderResponse>) => {
        ...
        finish(() => {
          ...
          const message = event.data.error || 'Infographic 渲染失败。';
          job.reject(
      ...
      worker.addEventListener('error', (event) => {
        finish(() => job.reject(new Error(event.message || 'Infographic Worker 意外退出。')));
      });
      worker.addEventListener('messageerror', () => {
        finish(() => job.reject(new Error('Infographic Worker 返回了无法读取的结果。')));
      });
...
export const renderInfographic = createInfographicRenderer();
