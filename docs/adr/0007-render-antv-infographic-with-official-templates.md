...
Fuxian accepts exact built-in template, theme, and named palette entries from the pinned runtime, bounded data, and a small safe theme-field allowlist. Static word-cloud, illustration, and `sequence-interaction` templates are supported; the latter describe sequence layouts and do not enable the official editor. It rejects arbitrary resource URLs, resource objects, arbitrary attributes, custom designs, unknown palette names, and animated templates. Animated templates remain disabled because the official exporter preserves indefinitely running SMIL rather than producing a deterministic PDF frame. Fuxian preserves the accepted Infographic source and output styling.
