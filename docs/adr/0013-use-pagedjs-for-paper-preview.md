...
The application starts in continuous mode. Paper mode is transient and always uses A4 portrait with `18 mm` block and `16 mm` inline margins. It scales each fixed page down so the whole sheet fits the available reading width and height, but never enlarges it beyond `100%`; this screen-only scaling does not change pagination. Existing A4 document width remains a continuous-layout preference.
