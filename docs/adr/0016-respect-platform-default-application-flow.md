...
Fuxian detects `.md` and `.markdown` associations independently in packaged Windows and macOS builds. Windows reads the current per-user `UserChoice` ProgID and compares it with the stable packaged `Fuxian.Markdown` ProgID. macOS asks Standard Additions for each extension's Launch Services default application and compares the resolved application bundle with the running bundle.
