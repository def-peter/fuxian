...
- Paged.js owns the page margins. The runtime-only native print stylesheet forces zero `@page` margins after pagination, preventing a second margin and automatic shrinking. PDF regression checks compare actual text coordinates and font sizes with paper preview across continuous document-width preferences, as well as page size and count.
- Block formulas use the same measured, atomic pagination placeholders as diagrams. Restore their original selectable MathML after pagination so the paginator cannot split formula internals and create premature page breaks. Inline formulas continue to flow with their surrounding text.
- Paper pagination begins only after fonts, images, formulas, and rendered visual blocks settle. A newer external revision or typography change supersedes older pagination; the previous successful pages remain visible until their replacement is complete. Tables retain their authored content and show headers only once. Measure column proportions on the complete table at printable width and preserve them across pages. Keep the header with the first data row when their combined height fits a page; ordinary rows stay intact, while an over-height row may continue across pages within its original columns. Pagination follows rendered height, never a fixed row count, and introduces no repeated labels or replacement prose.
...
- Mermaid uses the official HTML-label rendering path so Markdown emphasis, wrapping, and node geometry remain consistent across reading, focused viewing, paper preview, SVG copying, and PDF export.
...
- Open and recent document items offer a right-click context menu with “复制文件路径” (Copy File Path) first, which copies the item's exact absolute path, followed by “在访达中显示” on macOS / “在文件资源管理器中显示” on Windows (Reveal in Finder / Show in File Explorer). Keyboard users can invoke the same menu with the context-menu key or Shift+F10. There is no extra inline icon or overflow button: preserve filename space and the existing close/remove action. Revealing or copying another file never activates it, changes reading position, or updates recent history; reveal failures show a dismissible message. Unavailable recovery items keep their existing recovery actions. This resolves #57's prototype review in favor of A's context menu only; the comparison is archived on the local `codex/prototype-session-reveal` branch (`cf7e1c7`).
...
- In `正在查看`, dragging anywhere on an item except its close or recovery controls with the primary mouse button reorders open documents within that section. Pressing and moving does not activate another document; a simple click still does. Dropping elsewhere cancels; reordering preserves the active document, reading position, revision state, and recent history. `最近查看` does not initiate reordering.
...
- `Cmd/Ctrl+W` uses that same close-document action in the main window, including reading, paper preview, and source editing. Closing the last document leaves the start view; in Settings it closes only that window. `Cmd/Ctrl+Shift+W` closes the window while preserving its document session. Production menus do not expose renderer reload commands or their accelerators.
...
- Settings use a separate desktop window with general, appearance, document, PlantUML, Extensions, and About & Updates sections. General settings report `.md` and `.markdown` default-app status separately and only open the operating system's confirmation workflow after an explicit user action; the state refreshes when the window regains focus.
- About & Updates uses the full settings content area, with one brand header containing the logo, name, slogan, version, and Website/GitHub/Report an issue links, followed by software updates and diagnostics. Extensions also uses the full content area; other sections retain their document preview column. Sections share aligned headings, descriptions, and compact actions separated by subtle rules. Website and skill-guide links follow the interface language and open in the system browser.
- Extensions contains the companion `fuxian-diagram` skill, labeled `图表创作 Skill / Diagram authoring skill`. Explain that it is installed into external AI tools to guide writing Markdown diagram code. Its installation command is collapsed by default, can be copied, and is never executed by the app. Use a text action labeled `查看安装步骤 / Show installation steps`, changing to `收起安装步骤 / Hide installation steps` when expanded; avoid a bordered dropdown treatment. Show Node.js and terminal prerequisites when expanded, followed by an example AI prompt. Do not imply built-in AI generation or model configuration.
- About & Updates includes a quiet Diagnostics section with `导出诊断日志 / Export logs` and `清理日志 / Clear logs`. Export uses a native save dialog and clear immediately removes local diagnostic history without changing documents or viewing history. Show inline completion/failure feedback. Retain at most seven calendar days and 10 MB locally; exclude content, filenames, raw errors and URLs, and use anonymous file identifiers. Explain troubleshooting and local retention without implying an upload service.
- Packaged Windows and macOS builds check the stable update channel after a short non-blocking startup delay and every 24 hours while the application remains open. Development builds show that updates are unavailable instead of contacting the production feed.
...
- Available updates show the target version with download actions before release notes. Notes follow the interface language with fallback to the available language, retain inert headings, paragraphs, and lists, and scroll within a bounded region with a full-release-notes action. Download begins only after explicit confirmation, remains cancellable, and reports progress while the reader stays usable.
- A downloaded Windows update offers “重启并更新” and “稍后”. Normal application quit never installs it silently. Installation waits for document-session persistence and is refused while PDF export is active. macOS downloads a full DMG with explicit resume after cancellation or restart, then offers “打开安装包” for manual drag-and-drop replacement without quitting Fuxian. Both platforms display speed and provide “在 GitHub 下载” throughout supported update states; opening it never cancels an active download.
...
- Each newly available version also produces one restrained, non-modal reminder. Displaying it records that version in application update state; dismissing it does not clear the persistent Settings indicator or start a download, and a later version may remind again.
...
- Vega-Lite accepts the canonical `vega-lite` fence with JSON validated by the bundled official schema and compiler, including inline named datasets, data formatting, parameters, expressions, and transforms. It renders locally in cancellable workers with bounded input, dimensions, and SVG output. External data, image marks, and mark links remain outside the offline snapshot boundary. Parameters and selections render their initial state; browser input widgets and live brushing are not provided. Random/time-dependent expressions are frozen into each rendered snapshot.
- Vega-Lite `container` width follows the chart's available document content width; `container` height follows the independently sized reading viewport to avoid content-height feedback loops. Responsive charts rerender after container changes settle, while numeric and step-based sizes retain official behavior. Focused, paper, and PDF views reuse the resulting SVG snapshot; paper scaling never rewrites author input or recomputes random values.
- Vega-Lite preserves runtime-formatted tooltip values in its SVG snapshot. Hovering or focusing a chart mark shows the author's fields, titles, and number formats in continuous, paper, and focused views; `tooltip: null` remains disabled. One shared tooltip presentation follows document/shell tokens, dismisses on leaving, scrolling, or Escape, and stays out of PDF output. Pointer movement never recompiles a chart.
- AntV Infographic accepts the canonical `infographic` fence and passes the pinned runtime's official templates, animation, custom design, complete theme configuration, resource objects, data attributes, and sizing through a cancellable worker. The library editor remains outside the read-only finished document. Local Lucide/MDI resources are preferred, while online search and SVG loading are restricted to reviewed official-service and Alibaba CDN paths. Downloaded resources and final SVG are sanitized without remapping official layout or author styling; screen, full-screen view, SVG copying, paper preview, and PDF export share one sanitized snapshot.
...
- The interface defaults to the operating-system language: every `zh-*` locale uses Simplified Chinese, while all other or unknown locales use English. Readers may override this in Settings; the choice applies immediately to every window and persists across launches.
- The document-session header, start view, window title, and system menus use `浮现` in Chinese and `Fuxian` in English. File names, paths, Markdown source, finished-document content, and author-supplied diagram labels are never translated with the application shell.
- The application shell uses cool-gray side regions, a cool-white document surface, one-pixel separators, approximately four-pixel control radii, and shadows only for menus, drawers, dialogs, and other overlays. Dark appearance uses graphite cool grays rather than green-black surfaces; both appearances reserve green, amber, and red for semantic states.
...
- The application identity combines the standalone mark with a live, locale-specific product title: `浮现` for Chinese and `Fuxian` for English. The title remains UI text rather than part of an image so localization, scaling, and accessibility follow the application shell.
