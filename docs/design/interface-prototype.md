...
- Open and recent document items offer a right-click context menu with “在访达中显示” on macOS / “在文件资源管理器中显示” on Windows (Reveal in Finder / Show in File Explorer). Keyboard users can invoke the same menu with the context-menu key or Shift+F10. There is no extra inline icon or overflow button: preserve filename space and the existing close/remove action. Revealing another file never activates it, changes reading position, or updates recent history; failures show a dismissible message. Unavailable recovery items keep their existing recovery actions. This resolves #57's prototype review in favor of A's context menu only; the comparison is archived on the local `codex/prototype-session-reveal` branch (`cf7e1c7`).
