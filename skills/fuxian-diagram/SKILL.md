...
先读 [capabilities.md](references/capabilities.md) 确认 Fuxian 边界，再读取选中引擎的索引，并按其中的任务条件读取具体图类型指南：
...
按 [validation.md](references/validation.md) 检查内容、真实渲染、文档宽度和分页。发现裁切、重叠或字太小时，调整布局、精简标签或拆图，再渲染检查。

交付时简述选取的内容与图形理由，提供成品或 Markdown 路径，并说明实际验证范围。**源码检查、渲染成功、视觉排版通过是三种不同结果**；没有渲染环境时交付源码并明确标注“尚未渲染及视觉验证”。
