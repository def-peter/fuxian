...
test('opens an interactive article structure map derived from the content outline', async () => {
  ...
  try {
    ...
    await outline.getByRole('button', { name: '查看文章结构图' }).click();

    const dialog = window.getByRole('dialog', { name: '文章结构图' });
    await expect(dialog).toBeVisible();
    await expect(dialog.getByText('文章结构测试.md · 6 个标题')).toBeVisible();
    const svg = dialog.getByRole('img', { name: '当前文章的标题结构思维导图' });
    ...
    await expect(dialog.getByRole('button', { name: '缩小文章结构图' })).toBeEnabled();
    await dialog.getByRole('button', { name: '放大文章结构图' }).click();
    await dialog.getByRole('button', { name: '适应文章结构图窗口' }).click();
    ...
    await dialog.getByRole('button', { name: '关闭文章结构图' }).click();
